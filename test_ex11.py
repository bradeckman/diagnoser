...
def test_diagnose3():
    assert "influenza" == diagnoser3.diagnose(["cough", "fever"])
    assert "hard influenza" == diagnoser3.diagnose([])

def test_diagnose6():
    assert "cold" == diagnoser6.diagnose(["cough", "fever"])
...
def test_all_illnesses4():
    result = diagnoser4.all_illnesses()
    assert sorted(["influenza", "hard influenza", "cold"]) == sorted(result)

def test_all_illnesses6():
    result = diagnoser6.all_illnesses()
    assert ["cold"] == result
...
def test_build_tree2():
    ...
    tree2 = build_tree(records, [])

    assert "healthy" == tree2.get_data()
...
def test_build_tree_brad():

    records = []
    tree3 = build_tree(records, ["fever", "brad"])

    assert None == tree3.positive_child.positive_child.data
    assert None == tree3.positive_child.negative_child.data
    assert None == tree3.negative_child.positive_child.data
    assert None == tree3.negative_child.negative_child.data

def test_build_tree_none():

    records = []
    tree3 = build_tree(records, [])

    assert None == tree3.get_data()
    assert None == tree3.get_positive_child()
    assert None == tree3.get_negative_child()
...
def test_optimal_tree_brad():
    records = parse_data(r"small_data1.txt")
    tree = optimal_tree(records, ["healthy", "brad"], 0)

    assert "healthy" == tree.get_data()
    assert None == tree.get_positive_child()
    assert None == tree.get_negative_child()
